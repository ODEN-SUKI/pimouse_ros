...
class LightsensorTest(unittest.TestCase):
    ...
    def test_get_value(self):
        rospy.set_param('lightsensors_frequency',10)    #センサの値取得の周期を10Hzに
    ...
    def test_change_parameter(self):
        rospy.set_param('lightsensors_frequency',1)    #センサの値取得の周期を1Hzに
        time.sleep(2)                             #反映を待つ
        c_prev = self.count                       #callbackが呼ばれた回数を記録
        time.sleep(3) 
        ###コールバック関数が3秒間で最低1回、最高でも4回しか呼ばれてないことを確認###
        self.assertTrue(self.count == 4,"freq does not change")
